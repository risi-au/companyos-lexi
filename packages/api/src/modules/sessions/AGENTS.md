...
- `agent_sessions` in `packages/db/src/schema/sessions.ts`
  ...
  - completed wrap-up fields: nullable `summary` and nullable `citations` array using the memory `Citation` shape
...
- `completeSession(db, { sessionId, summary?, citations? }, actor)`: editor/agent. Sets `completed`, stores wrap-up summary/citations, bumps heartbeat, emits `session.completed`.
...
- `session.completed` payload: `sessionId`, `scopePath`, `summary`, `citations`
