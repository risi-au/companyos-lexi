...
- (M8-04/M8-07) Creation wizard intake: `ensureDraftIntakeForScope`, `findRelatedHistory`, `assembleIntakeExternalPack`, `submitIntakePacket`, `updateIntakePacket`, `approveIntakePacket`, `provisionFromIntakePacket`, template editor helpers, and queue listing manage scope-first `intake_packets`. Drafts store the required creation reason in `answers.reason`, related-history selections as references, and the exact external pack snapshot. All transitions emit `intake.*` events through `emitEvent`; provisioning delegates to `provisionScope`, then creates docs/tasks/wiki via existing services, saves the final packet as a report record, and writes `source-refs` when selected history exists.
