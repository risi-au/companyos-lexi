...
- (M8-04) Creation wizard intake: `ensureDraftIntakeForScope`, `submitIntakePacket`, `updateIntakePacket`, `approveIntakePacket`, `provisionFromIntakePacket`, template editor helpers, and queue listing manage scope-first `intake_packets`. All transitions emit `intake.*` events through `emitEvent`; provisioning delegates to `provisionScope`, then creates docs/tasks/wiki via existing services and saves the final packet as a report record.
