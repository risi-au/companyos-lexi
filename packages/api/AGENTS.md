...
- (M8-03/M10-03) Scoped memory: `recallMemory` returns raw active wiki/document snippets for the effective scope subtree, nearest ancestor wiki scope, and root `critical-facts`/`pattern-*` only, with the latest read-time document revision id on each hit when one exists. It never calls an LLM, never returns records, never widens grants, and logs redacted `recall_memory` usage. `getContextBundle` includes the root `critical-facts` page in every context profile with a fixed size cap.
