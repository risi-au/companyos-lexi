...
- (M8-03/M10-03/M10-02) Scoped memory: `recallMemory` returns raw active wiki/document snippets for the acting principal's personal scope, effective scope subtree, nearest ancestor wiki scope, and root `critical-facts`/`pattern-*` only, with the latest read-time document revision id on each hit when one exists. It never calls an LLM, never returns records, never widens grants, and logs redacted `recall_memory` usage. `getContextBundle` includes the root `critical-facts` page in every context profile with a fixed size cap.
...
- (M8-04/M8-07/M8-09/M10-02) Creation wizard intake: `ensureDraftIntakeForScope`, `findRelatedHistory`, `assembleIntakeExternalPack`, `submitIntakePacket`, `updateIntakePacket`, `approveIntakePacket`, `provisionFromIntakePacket`, template editor helpers, and queue listing manage scope-first `intake_packets`. Drafts store the required creation reason in `answers.reason`, related-history selections as references, the exact external pack snapshot, and scrubbed required credential names/notes in `answers.required_credentials`. External packs include up to five recent pages from the actor's personal wiki when present. All transitions emit `intake.*` events through `emitEvent`; provisioning delegates to `provisionScope`, then creates docs/tasks/wiki via existing services, seeds a `connection` doc with `{{credential:name}}` references when required credentials exist, saves the final packet as a report record, and writes `source-refs` when selected history exists.
- (M8-09) Credential vault: `setCredential`, `listCredentials`, `getCredentialValue`, and `deleteCredential` manage encrypted per-scope secret values. AES-256-GCM uses optional env `COS_VAULT_KEY`; missing/invalid keys keep the app bootable but reject reads/writes with `VaultNotConfiguredError`. Metadata list responses never include values or ciphertext. Value reads require agent/editor/admin/owner, emit `credential.accessed`, and update `last_accessed_at`.
- (M9-01) Ops health: `getOpsHealth` and `registerExternalCredential` are root-admin-only services for credential expiry, capability liveness, LiteLLM key probes, webhook/skills recency, run-log rows, and deduplicated `alert.fired` events. External credential rows store metadata only.
- (M3-03) Canvas: saveCanvas/getCanvas/listCanvases/archiveCanvas + size cap + events; exported for MCP/HTTP/UI.
- (M3-04/M10-03) Resident agent: runTurn (tool loop over LiteLLM + services), listConversations, getConversationMessages; agent_conversations + agent_messages tables. Injected LLM config; mocked in tests. Tool-loop doc hits from recall/search are surfaced as final-message citations and `agent.turn_completed.citationCount`. Events: agent.turn_completed.
