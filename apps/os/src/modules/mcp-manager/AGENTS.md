...
- Show each connection token's derived scoped memory access state.
...
- Memory access is display-only here: connection tokens show the API-derived `memoryAccess` field and grants remain unchanged.
